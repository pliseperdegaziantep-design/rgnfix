...
const advisorRule = [
  ...
  "Müşteri kaliteli veya üst segment ürün isterse önce Neo Fashion, daha güçlü kapatma veya premium beklentisi varsa Nano Pro öner.",
  "Karartma isteyen müşteriye yalnızca Nano Pro VR03 veya Nano Pro VR04 varyantlarını öner ve bunların karartma kumaş olduğunu açıkça belirt.",
  "Honeycomb serisini müşteri özellikle sormadıkça ilk öneri olarak sunma.",
  "Kesin fiyat uydurma; ölçüye göre fiyat için fiyat hesaplama bölümüne veya WhatsApp hattına yönlendir.",
  "Bu sohbet içinde adres, ad soyad, telefon, mahalle, sokak veya kapı numarası isteme.",
  "Müşteri satın almak isterse uygun seriyi kısaca öner ve sipariş için WhatsApp hattına yönlendir.",
  "Müşterinin sadece sorduğu soruya cevap ver. Sorulmayan konuya girme ve gereksiz takip sorusu sorma.",
...
function localAnswer(text: string): Message | null {
  ...
  const asksPrice = /(fiyat|kaç para|ücret|maliyet|uygun fiyat|ekonomik)/.test(normalized);
  ...
  if (asksBlackout) {
    return { role: "assistant", content: "Karartma için Nano Pro VR03 veya VR04 öneririm; bu iki varyant karartma kumaştır." };
  }

  if (asksMedium) {
    return { role: "assistant", content: "Orta kalite için Neo Fashion serisini öneririm; desenli ve dengeli bir seçenektir." };
  }

  if (asksPremium) {
    return { role: "assistant", content: "Kaliteli seçim için Neo Fashion, daha güçlü kapatma ve premium kullanım için Nano Pro öneririm." };
  }

  if (asksPrice) {
    return { role: "assistant", content: "Fiyatlandırmada ilk önerim Nova serisidir; kesin fiyat ölçünüze göre hesaplanır." };
  }

  if (wantsToBuy) {
    return { role: "assistant", content: "İhtiyacınıza uygun seriyi seçip sipariş için WhatsApp’tan bize ulaşabilirsiniz.", showWhatsApp: true };
  }
...
function normalizeAssistantContent(content: string) {
  ...
  return shortReply.length > 190 ? `${shortReply.slice(0, 187).trim()}...` : shortReply;
}
  ...
  return (
    <div className="container py-8 max-w-4xl">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto leading-6">Plise perde kumaş serilerimiz ve renk kartelamız hakkında bilgi alabilirsiniz. İhtiyacınıza göre kısa ve doğru ürün önerisi sunarım. Yalnızca sistemimizde bulunan ürün ve renkleri öneririm.</p>
      </div>
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto overscroll-contain p-4 space-y-4">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center min-h-full space-y-6">
                ...
                <div className="text-center space-y-2"><h3 className="font-semibold">İhtiyacınıza uygun seriyi seçelim</h3><p className="text-sm text-muted-foreground max-w-md">Öneriler yalnızca kendi kumaş serilerimiz ve renk kartelamızdan oluşur.</p></div>
